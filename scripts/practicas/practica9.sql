...
SELECT CITY,STATE_PROVINCE,STREET_ADDRESS FROM LOCATIONS
WHERE COUNTRY_ID = 'CA' OR COUNTRY_ID = 'US' AND LENGTH(STREET_ADDRESS) > 15;




--Muestra la longitud del nombre y el salario anual (por 14) para los
--empleados cuyo apellido contenga el carácter 'b' después de la 3ª posición.


SELECT FIRST_NAME,LAST_NAME, LENGTH(FIRST_NAME) AS "LONG NAME", SALARY * 14 AS "ANUAL SALARY" FROM EMPLOYEES
WHERE INSTR(LAST_NAME,'b',4) > 0;



--Averiguar los empleados que ganan entre 4000 y 7000 euros y que
--tienen alguna 'a' en el nombre. (Debemos usar INSTR y da igual que sea mayúscula que minúsculas) y que tengan comisión

SELECT FIRST_NAME || ' ' || LAST_NAME AS NAME, SALARY, COMMISSION_PCT FROM EMPLOYEES
WHERE SALARY > 4000 AND SALARY < 7000 AND INSTR(UPPER(FIRST_NAME),'A')  > 0 AND COMMISSION_PCT IS NOT NULL;
...
WHERE INSTR(FIRST_NAME,'S',1,1) > 0 OR INSTR(LAST_NAME,'S',1,1) > 0;
...
SELECT FIRST_NAME || ' ' || LAST_NAME AS "NAME", SALARY ,RPAD('*',SALARY / 1000,'*') AS "RANKING" FROM EMPLOYEES ORDER BY SALARY DESC;